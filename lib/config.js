...
/**
 * Resolve settings from package.json (or an explicit wpDeployer config, e.g. from
 * a standalone wp-deployer.json / --config file). Does not exit; returns an error
 * key if validation fails.
 * @param {object} pkg - Parsed package.json (must have name, version)
 * @param {object} [wpDeployerOverride] - wpDeployer settings to use instead of pkg.wpDeployer
 ...
 */
export function resolveSettings (pkg, wpDeployerOverride) {
  const defaults = getDefaults(pkg)
  const wpDeployer = wpDeployerOverride !== undefined
    ? wpDeployerOverride
    : (Object.prototype.hasOwnProperty.call(pkg, 'wpDeployer') ? pkg.wpDeployer : {})
